...
char* base64(const void* binaryData, int len, int* flen) {
  ...
  *flen = 4 * (len + pad) / 3;  // (len+pad) IS divisible by 3
  ...
  int i = 0, byteNo;  // result counter, and which byte we're on of the original
                      // source data.
  // I still need these variables after the loop
  for (byteNo = 0;
       byteNo <=
       len - 3;  // This loop is NOT entered for if there
  ...
  // The last 3 octets must be converted carefully as if len%3==1 or len%3==2 we
  // must "pretend" there are additional bits at the end.
  if (pad == 1) {
    unsigned char BYTE0 = bin[byteNo];
    unsigned char BYTE1 = bin[byteNo + 1];
  ...
  } else if (pad == 2)  // len%3==1 (1,4,7,10)
  {
    ...
    base64String[i++] = b64[SEXTET_A(BYTE0)];
    base64String[i++] =
        b64[(0x3 & BYTE0) << 4];  // "padded" by 0's, these 2 bits are still HI
                                  // ORDER BITS.
 ...
 * @param output The resulting base64 string
...
int base64_block(base64_block_data_t* input, base64_block_encoded_t* output) {
  ...
  switch (input->len) {
    ...
    case 1:
      ...
      output->c[0] = b64[SEXTET_A(BYTE0)];
      output->c[1] = b64[(0x3 & BYTE0) << 4];
      output->c[2] = '=';
      output->c[3] = '=';
      output->len = 4;
      break;
    ...
    case 2:
      ...
      output->c[0] = b64[SEXTET_A(BYTE0)];
      output->c[1] = b64[SEXTET_B(BYTE0, BYTE1)];
      output->c[2] = b64[(0xf & BYTE1) << 2];
      output->c[3] = '=';
      output->len = 4;
      break;
    ...
    case 3:
      ...
      output->c[0] = b64[SEXTET_A(BYTE0)];
      output->c[1] = b64[SEXTET_B(BYTE0, BYTE1)];
      output->c[2] = b64[SEXTET_C(BYTE1, BYTE2)];
      output->c[3] = b64[SEXTET_D(BYTE2)];
      output->len = 4;
      break;
...
int unbase64_block(base64_block_encoded_t* input, base64_block_data_t* output) {
  ...
  switch (input->len) {
    case 0:
      // There is nothing to decode if the base64 string is empty.
...
int base64integrity(const char* ascii, int len) {
  ...
  // (inside an octet stream) to be VALID, THE LENGTH HAS TO BE A MULTIPLE OF 4.
  ...
  for (i = 0; i < len - 2; i++) {
    if (!isbase64ValidChr(ascii[i])) {
      printf(
          "ERROR in base64integrity at chr %d [%c]. String is NOT valid "
          "base64.\n",
          i, ascii[i]);
      return 0;
    }
  ...
  } else if (!isbase64ValidChr(ascii[i]))  // not = or valid base64
  {
    ...
    printf(
        "ERROR in base64integrity at chr %d (2nd last chr). String is NOT "
        "valid base64.\n",
        i);
    return 0;
  }
  ...
  if (ascii[i] != '=' && !isbase64ValidChr(ascii[i])) {
    printf(
        "ERROR in base64integrity at chr %d (last chr). String is NOT valid "
        "base64.\n",
        i);
    return 0;
  }
